...
const createMorphGeometry = () => {
    ...
    const planes =
        createDodecahedronPlanes();

    const facetedPositions =
        new Float32Array(
            position.count * 3
        );
...
const SphereModal = () => {
    ...
    const initialIsHomeRef =
        useRef(
            pathname === "/"
        );

    const initialMorphRef =
        useRef(
            initialIsHomeRef.current
                ? 0
                : 1
        );
    ...
    useFrame(() => {
        ...
        const solidInfluences =
            solid.morphTargetInfluences;

        const wireInfluences =
            wire.morphTargetInfluences;

        if (solidInfluences) {
            solidInfluences[0] =
                morphState.current.value;
        }
        ...
        >
            ...
            >
                <meshPhysicalMaterial
                    ...
                    transparent
                    opacity={
                        initialSolidOpacityRef.current
                    }
                />
            </mesh>
